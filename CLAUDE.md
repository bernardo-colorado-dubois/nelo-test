...
- **`src/spark_session.py`**: builder único de `SparkSession`, reutilizado por ambos scripts (ver "Stack Airflow + Spark" para cómo decide entre `local[*]` y el cluster real).
...
`src/spark_session.py::get_spark` lee la env var `SPARK_MASTER_URL`: si está seteada (el stack la fija a `spark://spark-master:7077`), se conecta al cluster real; si no (host, `make pipeline`), sigue usando `local[*]` — es el mismo `get_spark()` para ambos modos, sin ramas de código en los scripts.
...
1. **`spark.master(...)` explícito en código pisa el `--master` de `spark-submit`.** Por eso `get_spark()` nunca hardcodea `local[*]`: lee `SPARK_MASTER_URL` del entorno y arma el `.master(...)` correcto para cada contexto (ver arriba). Si algún job nuevo llama a `SparkSession.builder` directo sin pasar por `get_spark()`, va a ignorar el cluster real y correr en modo local dentro del contenedor — no hacer eso.

2. **Primera corrida del DAG falló con `java.io.IOException: Failed to rename ... part-00000....snappy.parquet`** al escribir la tabla Parquet — mismo bug ya documentado en el stack de referencia (commit protocol v1 de Spark falla al renombrar sobre volúmenes bind-mounted de Docker, sin relación con permisos). **Fix aplicado en `get_spark()`**: `spark.hadoop.mapreduce.fileoutputcommitter.algorithm.version=2` + `spark.hadoop.fs.permissions.umask-mode=000`. No sacarlos si se toca `spark_session.py`.
