...
1. **Local** (`make pipeline`): Spark `local[*]` sobre el venv del host, sin Docker. Ver "Comandos" abajo. Son solo los tres pasos de Spark — no sube nada a Drive.
2. **Orquestado** (`make stack-up`): los mismos tres scripts, sin modificar, enviados como jobs al cluster de Spark standalone vía un DAG de Airflow, más una cuarta tarea (`upload_to_drive`, Python puro, sin Spark) que sube el contenido de `output/items_flat.csv` a una Google Sheet nativa en Drive. Ver "Stack Airflow + Spark (Docker)" más abajo.
...
SQS (data-engineering-case-analytics-queue)
  ...
  → dags/upload_to_drive.py       → Google Sheet en Drive (misma hoja, contenido reemplazado cada corrida) [solo orquestado]
```
...
- `dags/upload_to_drive.py` (**cuarta tarea, solo orquestada — no vive en `spark/` ni corre con `make pipeline`**): no es un job de Spark, no arma `SparkSession` — es una función Python plana (`upload_to_drive(csv_path, drive_filename)`) que un `PythonOperator` del DAG llama después de `export_csv`. 1) credenciales de una cuenta de servicio de Google (`GOOGLE_APPLICATION_CREDENTIALS`, JSON montado de solo lectura, nunca commiteado — ver `credentials/` y `.gitignore`), 2) busca en `GOOGLE_DRIVE_FOLDER_ID` un archivo con el mismo nombre, 3) si existe lo sobreescribe (`files().update`, mismo file ID); si no, lo crea. En los dos casos el `body` fija `mimeType="application/vnd.google-apps.spreadsheet"` junto al `media_body` en formato csv — la propia API de Drive hace la conversión CSV → Google Sheet nativa al importar el contenido, así que lo que queda en Drive es una hoja de cálculo abrible/editable ahí mismo, no un archivo `.csv` descargable. No versiona nada: cada corrida deja una única Sheet actualizada, igual que `export_csv.py` deja un único CSV local.
...
`dags/etl_pipeline.py` tiene un solo DAG (`etl_pipeline`, `schedule=None`, disparo manual) con tres `SparkSubmitOperator` encadenados (`conn_id="spark_default"`, que resuelve a `spark://spark-master:7077?deploy_mode=client` vía la env var `AIRFLOW_CONN_SPARK_DEFAULT`): `read_queue` (args `--table-path /opt/spark-data/raw_messages --max-messages-per-poll 10 --wait-time-seconds 20`) → `transform_messages` (args `/opt/spark-data/raw_messages /opt/spark-data/items_flat`) → `export_csv` (args `/opt/spark-data/items_flat /opt/spark-output/items_flat.csv`) → `upload_to_drive` (`PythonOperator`, sin args de Spark: llama directo a `upload_to_drive(csv_path="/opt/spark-output/items_flat.csv", drive_filename="nelo_dashboard")`, importada desde `dags/upload_to_drive.py`, sibling del DAG). `MAX_MESSAGES_PER_POLL`/`WAIT_TIME_SECONDS` son parámetros operativos del poll a SQS que decide el DAG, no constantes fijas en `read_queue.py` — el único valor que sigue hardcodeado ahí es `VisibilityTimeout=120`, porque es un invariante de diseño (cubre el tiempo de la propia corrida hasta el borrado, ver punto 9 de "Decisiones no obvias") y no algo que el DAG deba poder cambiar.

`upload_to_drive` corre como `PythonOperator` (no `SparkSubmitOperator`) porque no es un job de Spark: sube un archivo ya generado, no toca DataFrames. Ejecuta como driver de Airflow dentro de `airflow-worker-1` — el mismo contenedor que ya tiene montado `/opt/spark-output` (el CSV) y, agregado para esta tarea, `/opt/google-drive-service-account.json` (la key de la cuenta de servicio, de solo lectura) — ver `docker-compose.yaml`. `dags/etl_pipeline.py` importa la función con `from upload_to_drive import upload_to_drive`: funciona porque Airflow agrega el directorio del propio DAG (`dags/`) a `sys.path` al parsearlo, así que un módulo sibling ahí se importa directo, sin necesitar `PYTHONPATH` extra ni vivir en `src/`.
...
Airflow UI → http://localhost:8080 (`airflow`/`airflow` por defecto, en `.env`). Spark master UI → :8081, workers → :8082+. Validado de punta a punta: `make stack-trigger` corrió `etl_pipeline` completo (`read_queue` → `transform_messages` → `export_csv` → `upload_to_drive`, las cuatro `success`) contra el cluster real y contra Drive real (ver punto 11 de "Decisiones no obvias" para la puesta en marcha de la cuarta tarea).
...
`GOOGLE_DRIVE_FOLDER_ID` (cuarta tarea del DAG) es la única variable propia de `upload_to_drive` — la otra mitad de esa credencial es el archivo `credentials/google-drive-service-account.json` (gitignored, nunca en `.env`), montado por `docker-compose.yaml` en `/opt/google-drive-service-account.json` y expuesto como `GOOGLE_APPLICATION_CREDENTIALS`. Dos requisitos, ninguno verificable por el código, los dos cubiertos en detalle en el punto 11 de "Decisiones no obvias": (1) la carpeta de Drive apuntada por `GOOGLE_DRIVE_FOLDER_ID` tiene que estar compartida como Editor con el `client_email` de esa key, y (2) tiene que existir ya adentro una Google Sheet en blanco con el nombre que pasa `drive_filename` (hoy `"nelo_dashboard"`) — la cuenta de servicio nunca puede crear el archivo, solo actualizar uno que ya sea nativamente una Sheet.
...
10. **La key de la cuenta de servicio de Google llegó al repo suelta en la raíz (`data-engineer-study-*.json`), sin trackear pero tampoco en `.gitignore`** — un `git add -A`/`git add .` la hubiera commiteado tal cual, secreto incluido. Se movió a `credentials/google-drive-service-account.json` (carpeta nueva, mismo patrón que `logs/`/`plugins/`/`config/`: `credentials/*` + `!credentials/.gitkeep` en `.gitignore`) y `docker-compose.yaml` la monta desde ahí, de solo lectura. Cualquier credencial nueva que se agregue al proyecto va ahí, nunca suelta en la raíz — la raíz no tiene ninguna regla de `.gitignore` que la cubra por default.

11. **Puesta en marcha real de `upload_to_drive` — cuatro fallas encadenadas antes de andar, ninguna por un bug del script:**
    - `403 accessNotConfigured` ("Google Drive API has not been used in project ... or it is disabled") — la Drive API nunca se había habilitado en el proyecto de GCP de la cuenta de servicio. Se habilita una sola vez desde `https://console.developers.google.com/apis/api/drive.googleapis.com/overview?project=<project_number>` (botón "Habilitar"), tarda un par de minutos en propagarse.
    - `403 storageQuotaExceeded` al intentar crear el archivo. Causa: **una cuenta de servicio no tiene cuota de almacenamiento propia en Drive** — esto vale para *cualquier* tipo de archivo que intente crear (probado también con `mimeType` de Google Sheet, mismo error: "The user's Drive storage quota has been exceeded"). Puede *actualizar* el contenido de un archivo que ya existe (la cuota la paga el dueño real del archivo), pero nunca *crear* uno nuevo en una carpeta de Drive personal.
    - `400 invalidContentType` ("Invalid MIME type provided for the uploaded content") al intentar convertir el `items_flat.csv` binario ya existente en una Google Sheet nativa vía `files().update(..., body={"mimeType": "application/vnd.google-apps.spreadsheet"}, ...)`. Causa: **Drive no permite convertir un archivo binario ya creado a un tipo nativo de Google Docs/Sheets/Slides mediante `update`** — esa conversión (CSV → Sheet) solo la aplica en el momento de `create`, nunca después sobre un archivo que ya nació como blob.
    - Combinando las dos anteriores: como una Sheet nueva tampoco se puede *crear* con la cuenta de servicio (mismo límite de cuota) y un CSV existente tampoco se puede *convertir* a Sheet, la única combinación que funciona es que **el archivo ya exista en Drive y ya sea nativamente una Google Sheet** antes de que la cuenta de servicio lo toque — ahí sí, `files().update(fileId=..., media_body=<csv>)` reimporta el csv como datos de esa hoja sin cambiar de tipo ni consumir cuota de nadie más que el dueño real.
    - **Fix operativo final, no de código**: crear a mano, una sola vez, una Google Sheet en blanco dentro de la carpeta de Drive (Drive → Nuevo → Hojas de cálculo de Google — **nunca** subir un `.csv`), con el mismo nombre que espera `upload_to_drive` (`drive_filename` en `dags/etl_pipeline.py`, hoy `"nelo_dashboard"` — el nombre es arbitrario, lo único que importa es que exista y sea del tipo correcto). Si ese archivo se llega a borrar de Drive, hay que repetir este paso antes de la siguiente corrida.
