...
Ninguno de los dos tiene `run()` ni `main()`: son procedimientos planos bajo `if __name__ == "__main__":` — nada más los llama ni hace falta reusarlos desde otro código Python, así que esas funciones no aportaban nada. Ambos corren igual como script local o como aplicación de `spark-submit`.
