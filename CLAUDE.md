...
11. **Primera corrida real de `upload_to_drive` falló dos veces antes de andar, ninguna de las dos por un bug del script:**
    - Primero con `403 accessNotConfigured` ("Google Drive API has not been used in project ... or it is disabled") — la Drive API nunca se había habilitado en el proyecto de GCP de la cuenta de servicio. Se habilita una sola vez desde `https://console.developers.google.com/apis/api/drive.googleapis.com/overview?project=<project_number>` (botón "Habilitar"), tarda un par de minutos en propagarse.
    - Después, ya con la API habilitada, con `403 storageQuotaExceeded` al intentar crear el archivo. Causa: **una cuenta de servicio no tiene cuota de almacenamiento propia en Drive** — puede *actualizar* el contenido de un archivo que ya existe (la cuota la paga el dueño real del archivo), pero no puede *crear* un archivo nuevo en una carpeta de Drive personal (ahí el dueño sería la cuenta de servicio, con 0 bytes de cuota). Como la carpeta destino estaba vacía, `upload_to_drive.py` no encontraba ningún `items_flat.csv` existente y caía siempre por la rama de `files().create(...)`. **Fix operativo, no de código**: crear a mano, una sola vez, un `items_flat.csv` vacío en la carpeta de Drive (desde la cuenta de Google real, dueña de la carpeta) — a partir de ahí, `upload_to_drive.py` siempre lo encuentra por nombre y actualiza su contenido (`files().update`), que sí es una operación permitida para una cuenta de servicio sin cuota propia. Si el archivo destino se llega a borrar de Drive, hay que repetir este paso una vez más antes de la siguiente corrida.
